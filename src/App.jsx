...
function Dashboard() {
  ...
  useEffect(()=>{if(!companyId)return;load();async function load(){const [c,ch,p]=await Promise.all([supabase.from("customers").select("id",{count:"exact",head:true}).eq("company_id",companyId),supabase.from("charges").select("*,customers(name,phone)").eq("company_id",companyId).order("due_date"),supabase.from("payments").select("*").eq("company_id",companyId)]);setData({customers:c.count||0,receive:(ch.data||[]).filter(x=>x.status==="pending").reduce((a,x)=>a+Number(x.amount),0),today:(ch.data||[]).filter(x=>x.status==="pending"&&x.due_date===todayISO()).reduce((a,x)=>a+Number(x.amount),0),overdue:(ch.data||[]).filter(x=>x.status==="pending"&&x.due_date<todayISO()).reduce((a,x)=>a+Number(x.amount),0),paid:(p.data||[]).reduce((a,x)=>a+Number(x.amount),0),charges:ch.data||[]});}}
