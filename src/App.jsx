...
function ThemeToggle(){const [dark,setDark]=useState(()=>localStorage.getItem("cobrancapro-theme")==="dark");useEffect(()=>{document.documentElement.classList.toggle("dark",dark);localStorage.setItem("cobrancapro-theme",dark?"dark":"light");},[dark]);return <button type="button" className="theme-toggle" onClick={()=>setDark(x=>!x)} aria-label={dark?"Ativar modo claro":"Ativar modo escuro"} title={dark?"Modo claro":"Modo escuro"}><span>{dark?"☀":"☾"}</span></button>}
